...
<div id="classement-list" aria-live="polite">
    ...
    <!-- Contenu des onglets -->
    <div>
            <!-- Onglets: Classement public / Vote Jour J -->
            <div class="mb-4 flex items-center justify-center gap-3">
                <button @click.prevent="tab='general'" :class="tab==='general' ? 'bg-yellow-400 text-black' : 'bg-gray-800 text-white'" class="px-4 py-2 rounded-md">Classement public</button>
                <button @click.prevent="tab='jourj'" :class="tab==='jourj' ? 'bg-rose-500 text-white' : 'bg-gray-800 text-white'" class="px-4 py-2 rounded-md">Vote Jour J</button>
            </div>
        ...
        <!-- Classement Jour J -->
        <div x-show="tab === 'jourj'" class="space-y-4" style="display: none;">
            @if(isset($classementJourJ) && $classementJourJ->count())
                @foreach ($classementJourJ as $index => $projet)
                    @php $rank = ($classementJourJ->firstItem() ?? 0) + $index; @endphp
                    @include('classement-item', ['projet' => $projet, 'rank' => $rank])
                @endforeach

                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-3 mt-2">
                    <div class="flex flex-wrap items-center gap-3 sm:gap-4">
                        <p class="mb-0 text-sm text-gray-300">Affichage {{ $classementJourJ->firstItem() }}–{{ $classementJourJ->lastItem() }} sur {{ $classementJourJ->total() }}</p>
                    </div>

                    <nav class="flex items-center flex-wrap gap-2" role="navigation" aria-label="Pagination">
                        @if($classementJourJ->previousPageUrl())
                            <a href="{{ $classementJourJ->previousPageUrl() }}" rel="prev" class="inline-flex items-center justify-center w-9 h-9 rounded-md bg-gray-800 hover:bg-gray-700 text-white" aria-label="Page précédente">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M12.293 16.293a1 1 0 0 1-1.414 1.414l-6-6a1 1 0 0 1 0-1.414l6-6a1 1 0 0 1 1.414 1.414L7.414 10l4.879 4.879z" clip-rule="evenodd"/></svg>
                            </a>
                        @endif

                        @php
                            $lastPage = $classementJourJ->lastPage();
                            $current = $classementJourJ->currentPage();
                            $pages = [1, $current - 1, $current, $current + 1, $lastPage];
                            $pages = array_values(array_unique(array_filter($pages, fn($p) => $p >= 1 && $p <= $lastPage)));
                            sort($pages);
                        @endphp
                        <ul class="flex items-center ml-1 space-x-2">
                            @for ($i = 0; $i < count($pages); $i++)
                                @if($i > 0 && $pages[$i] - $pages[$i-1] > 1)
                                    <li class="text-gray-500 text-sm">…</li>
                                @endif
                                @php $p = $pages[$i]; $url = $classementJourJ->url($p); $isActive = $current == $p; @endphp
                                <li>
                                    <a href="{{ $url }}" class="px-3 py-1 rounded-md text-sm {{ $isActive ? 'bg-yellow-400 text-black' : 'bg-gray-800 text-white hover:bg-gray-700' }}" @if($isActive) aria-current="page" @endif>{{ $p }}</a>
                                </li>
                            @endfor
                        ...
                        @if($classementJourJ->nextPageUrl())
                            <a href="{{ $classementJourJ->nextPageUrl() }}" rel="next" class="inline-flex items-center justify-center w-9 h-9 rounded-md bg-gray-800 hover:bg-gray-700 text-white ml-3" aria-label="Page suivante">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M7.707 3.707a1 1 0 0 0-1.414-1.414l-6 6a1 1 0 0 0 0 1.414l6 6a1 1 0 0 0 1.414-1.414L3.414 10l4.293-4.293z" clip-rule="evenodd"/></svg>
                            </a>
                        @endif
                    </nav>
            ...
            @else
                <div class="text-center p-12 text-gray-400">
                    <p class="text-xl">Aucun vote Jour J enregistré pour le moment.</p>
                </div>
            @endif
        </div>

        <!-- Removed per-category sections: only public classement and jour j are shown -->
    </div>
</div>
